...
router.get("/posts/featured", authenticateToken, async (req, res) => {
  try {
    const range = req.query.range || "day"; // "day" hoặc "week"
    const limit = Number(req.query.limit) || 5;

    // Tính toán mốc thời gian lọc
    let timeCondition = "p.created_at >= DATE_SUB(NOW(), INTERVAL 1 DAY)";
    if (range === "week") {
      timeCondition = "p.created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY)";
    }

    // Tiêu chí Nổi bật: Tính điểm dựa trên số Like và Comment
    const [featuredPosts] = await pool.query(
      `
        ...
        LEFT JOIN users u ON p.user_id = u.id
        WHERE ${timeCondition}
        ORDER BY (p.likes * 2 + p.comments_count) DESC, p.created_at DESC
        LIMIT ?
      `,
      [limit]
    );

    // Xử lý lại tags từ JSON string sang mảng trước khi trả về
