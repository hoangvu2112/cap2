...
router.patch("/:id/status", authenticateToken, requireRole("user", "dealer"), async (req, res) => {
  try {
    ...
    // Nếu chốt giao dịch -> tạo order + payment (farmer trả phí)
    if (status === "closed") {
      // Thực hiện transaction với retry khi gặp deadlock
      const maxAttempts = 3
      let attempt = 0
      ...
      while (attempt < maxAttempts) {
        ...
        try {
          ...
          // Lấy lại dữ liệu đầy đủ của yêu cầu
          const [[reqRow]] = await connection.query(
            `SELECT id, buyer_id, farmer_id, product_id, quantity, proposed_price, status FROM purchase_requests WHERE id = ? FOR UPDATE`,
            [requestId]
          )
          ...
          // Tránh tạo duplicate: kiểm tra đã có order liên quan chưa
          const [[existingOrder]] = await connection.query(
            `SELECT id FROM orders WHERE purchase_request_id = ? LIMIT 1`,
            [requestId]
          )

          if (existingOrder) {
            // Update trạng thái yêu cầu rồi trả về
            await connection.query("UPDATE purchase_requests SET status = ? WHERE id = ?", [status, requestId])
          ...
          // Tính toán tiền
          const totalAmount = Number(reqRow.quantity || 0) * Number(reqRow.proposed_price || 0)
          const totalFee = (function (amount) {
          ...
          // Tạo order (bỏ cột status để dùng default DB và tránh lỗi enum/schema khác nhau)
          const [orderResult] = await connection.query(
            `INSERT INTO orders (purchase_request_id, product_id, farmer_id, dealer_id, quantity, price_per_unit, total_amount, total_fee, farmer_fee, dealer_fee)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [requestId, reqRow.product_id, reqRow.farmer_id, reqRow.buyer_id, reqRow.quantity, reqRow.proposed_price, totalAmount, totalFee, farmerFee, dealerFee]
          )

          const orderId = orderResult.insertId

          // Tạo payment cho nông dân (farmer) - pending
          const [paymentResult] = await connection.query(
            `INSERT INTO payments (user_id, amount, payment_type, status, reference_id, note)
             VALUES (?, ?, 'transaction', 'pending', ?, ?)`,
            [reqRow.farmer_id, farmerFee, orderId, `Transaction fee for order ${orderId}`]
          )

          // Commit and update purchase_request status
          await connection.query("UPDATE purchase_requests SET status = ? WHERE id = ?", [status, requestId])
          await connection.commit()
          connection.release()

          return res.json({ message: "Đã chốt giao dịch, order & payment tạo pending", status, orderId, totalAmount, totalFee, farmerFee, dealerFee })
        } catch (err) {
          await connection.rollback().catch(()=>{})
          connection.release()
          // Nếu deadlock thì thử lại vài lần
          console.error("Error creating order/payment on closing request:", err)
          if (err && (err.code === 'ER_LOCK_DEADLOCK' || err.errno === 1213)) {
            if (attempt < maxAttempts) {
              const backoff = 100 * attempt
              console.log(`⚠️ Deadlock detected, retrying in ${backoff}ms (attempt ${attempt}/${maxAttempts})`)
              await sleep(100 * attempt)
              continue
            }
          }
          return res.status(500).json({ error: "Không thể tạo order/payment khi chốt giao dịch" })
        }
      }
      // Nếu vòng lặp kết thúc mà chưa return thì báo lỗi chung
      return res.status(500).json({ error: "Không thể tạo order/payment khi chốt giao dịch" })
    }

    // Trường hợp không phải closed (không cho update từ client)
    await pool.query(
